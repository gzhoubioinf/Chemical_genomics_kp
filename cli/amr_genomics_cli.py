...
def load_config(config_path):
    ...
    # No variable substitution needed as absolute paths are used
    return raw_config
...
def load_xgb_models_for_condition(condition, MODEL_PATHS):
    """Load XGBoost models for the given condition."""
...
def compute_stats(df, prediction, param_name):
    ...
    if std_val != 0:
        s_score = (prediction - mean_val) / std_val
    else:
        s_score = np.nan
    ...
    if mean_val != 0:
        pct_diff = ((prediction - mean_val) / mean_val) * 100
    else:
        pct_diff = np.nan
    ...
    for rec in all_ref_records:
        ...
        for pc_idx, unitigs in target_unitigs.items():
            for utg in unitigs:
                ...
                # Forward reference, forward unitig
                ...
                # Forward reference, reverse unitig
                ...
                # Reverse reference, forward unitig
                ...
                # Reverse reference, reverse unitig
...
def colony_picker(args, config):
    try:
        ...
        # Load relevant files from config
        strain_file_path = config["files"]["strain_file"]
        ...
        # Map strain name to row and column
        strain = args.strain
        strain_row_col = strains_df[strains_df['ID'] == strain]
        if strain_row_col.empty:
            summary_file.write(f"ERROR: Strain {strain} not found in strain file.\n")
            summary_file.close()
            sys.exit(1)
        ...
        # Load gene‐antibiotic map
        try:
            with open(gene_antibiotics_file, 'r') as f:
                if gene_antibiotics_file.endswith('.yaml') or gene_antibiotics_file.endswith('.yml'):
        ...
        # Write text summary to .out file
        ...
        # Gather replicate data in memory, so we can build a single multi-subplot figure
        rep_values = {"circularity": [], "size": [], "opacity": []}
        replicate_images = []  # Store tuples (rep_label, rgb_image_array)
        ...
        for rep in available_reps:
            ...
            if extracted is not None:
                ...
                # Instead of saving each image, we store it in a list
        ...
        # ---------------------------------------------------------------------
        # 1) Create a single figure for the replicate images
        # ---------------------------------------------------------------------
        if replicate_images:
            ...
            # If there's only one replicate, axes won't be a list
        ...
        # ---------------------------------------------------------------------
        # 2) Create a single figure with 3 subplots for the distribution plots
        # ---------------------------------------------------------------------
            ...
            if valid_vals:
                ...
                for v, rp, clr in zip(vals, available_reps, colors):
                    if v is not None:
                        ...
                        # Evaluate the density at point v
...
def ml_prediction(args, config):
    ...
    and SHAP-based PCA analysis. All results saved to /ibex/user/hinkovn/AMR_Genomics_KP_Results.
    Also writes numerical/statistics to a .out file in the same directory.
    """
    try:
        output_dir = "/ibex/user/hinkovn/AMR_Genomics_KP_Results"
        ensure_dir(output_dir)
        ...
        # Load XGBoost models
        models = load_xgb_models_for_condition(condition, MODEL_PATHS)
        model_opacity = models.get("opacity")
        model_circ    = models.get("circularity")
        model_size    = models.get("size")

        # We'll write all results for these FASTAs to a single .out file
        ...
        for fasta_path in args.fasta:
            ...
            # 1) Check known resistance/virulence genes in user FASTA
            try:
            ...
            # 2) Convert to unitig vector and predict
            ...
            try:
                ...
                num_pcs_to_show = 10  # Adjust as needed
                ...
                for pc_idx in top_pc_indices:
                    ...
                    top_idxs = np.argsort(abs_loadings)[::-1][:5]  # Top 5 unitigs
